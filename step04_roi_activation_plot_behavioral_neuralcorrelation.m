...
clear;
format long g;
roistr='hippocampusleft_05correct';
roistatsfile=['R:\qinproject\analysis20220409\glmanalysis\glmgroupresults_29rois\',roistr,'_percentageofchange.mat'];
load(roistatsfile);
load('R:\qinproject\analysis20220409\glmanalysis\glmgroupresults_29rois\accandrt.mat')
data=[];
nsubjects=size(roiactivation.results,1);
load('R:\qinproject\datamostoriginal\onsets\dst_design_shortblock_correct20211114.mat');

for i=1:nsubjects
    data(i,1)=roiactivation.results(i,1);
    data(i,2)=roiactivation.results(i,2);
    data(i,3)=roiactivation.results(i,3);
    ...
    data(i,6)=behavioraldata.rt(indinaccrt,2);
    data(i,7)=behavioraldata.rt(indinaccrt,3);
    ...
    data(i,11)=data(i,6)-data(i,7);  % rt emotion - shape
end
...
xlabel('\Delta %BOLD change','FontSize',20);
% ctick = get(gca, 'xTick');
